...
function closestPointOnLineLLA(targetLLA, cameraLLA, distance) {
...
window.applyFollowCam = function (e, o, t) {
    ...
    var _ = M33.transform(v, geofs.camera.worldPosition)
        , b = V3.scale(v[1], -o.distance);
    if (geofs.preferences.camera.followMotion) {
        let x = V3.exponentialSmoothing("elasticcam", t.rigidBody.v_acceleration, 1, t.rigidBody.v_acceleration, e);
        _ = V3.sub(_, V3.scale(x, .1 * t.definition.motionSensitivity)),
            b = V3.sub(b, V3.scale(x, .2 * t.definition.motionSensitivity))
    }
    var S = V3.add(t.llaLocation, xyz2lla(_, t.llaLocation))
        , P = xyz2lla(b, S);
    
    geofs.camera.lla = betterEnabled ? closestPointOnLineLLA(geofs.aircraft.instance.llaLocation, geofs.camera.lla, o.distance) : V3.add(S, P),
