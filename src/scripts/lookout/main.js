(function () {
    ...
    // ─── Silk instances ────────────────────────────────────────────────────────
    let pitchSilk = new Silk(0, {
        min: config.pitch.min,
        max: config.pitch.max,
    });
    ...
    let leftRightSilk = new Silk(0, {
        min: config.leftRight.min,
        max: config.leftRight.max,
    });
    //let forwardBackwardSilk = new Silk(0, { min: config.forwardBackward.min, max: config.forwardBackward.max });
    let upDownSilk = new Silk(0, {
        min: config.upDown.min,
        max: config.upDown.max,
    });
    ...
    geofs.camera.setRotation = function (e, t, a) {
        ...
        return "follow" == geofs.camera.currentModeName ||
            "fixed" == geofs.camera.currentModeName
    ...
    geofs.api.viewer.scene.preRender.addEventListener(() => {
        ...
        if (mouseDownOrientation === null && controls.mouse.down) {
            mouseDownOrientation = [
                ...geofs.camera.currentDefinition.orientations.current,
            ];
        }
        ...
        if (mouseDownOrientation) {
            ...
            pitchSilk.setCalibrationValue(
                pitchSilk.calibrationValue + change[1],
            );
